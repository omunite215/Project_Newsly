...
// Custom hook to use snackbar in mutations
...
export const useUpvotePost = () => {
	...
	const { enqueueSnackbar } = useSnackbarMutation();
	...
	return useMutation({
		...
		onError: (err, variable, context) => {
			...
			enqueueSnackbar("Failed to upvote post", {
				variant: "error",
...
export const useUpvoteComment = () => {
	...
	return useMutation({
		...
		onError: (err, { parentCommentId, postId }, context) => {
			...
			console.error(err);
			enqueueSnackbar("Failed to upvote comment", {
				variant: "error",
				autoHideDuration: 3000,
			});
			if (context?.prevData) {

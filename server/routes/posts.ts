...
export const postRouter = new Hono<Context>()
	.post("/", loggedIn, vValidator("form", createPostSchema), async (c) => {
	...
	.post(
		"/:id/upvote",
		loggedIn,
		vValidator("param", checkIdSchema),
		async (c) => {
	...
	.post(
		...
		loggedIn,
		vValidator("param", checkIdSchema),
		vValidator("form", createCommentSchema),
		async (c) => {
			const { id } = c.req.valid("param");
	...
	.get(
		"/:id/comments",
		vValidator("param", checkIdSchema),
		vValidator("query", commentPaginationSchema),
		async (c) => {
			const user = c.get("user") as User;
			const { id } = c.req.valid("param");
	...
	.get("/:id", vValidator("param", checkIdSchema), async (c) => {
		const user = c.get("user");
		const { id } = c.req.valid("param");
		const postsQuery = db
			.select({

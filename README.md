# Project_Newsly

A modern full stack project made using bun and hono.

Step-1 - Project Setup (completed).
Step-2 - Drizzle Setup (completed)
Step-3 - Hono Error Handler (completed)
Step-4 - Lucia Auth (completed)
Step-5 - Tables Setup (completed)
Step-6 - Posts Route (completed)
Step-7 - Comments Route (completed)
Step-8 - Frontend Setup (completed)
Step-9 - Layouts and Helpers (completed)
Step-10 - Auth Routes (completed)
Step-11 - Homepage Posts (completed)
Step-12 - Submit Post Form (completed)
Step-13 - Setup Posts Page (completed)
Step-14 - Loaders and Checks (completed)
Step-15 - Fixing Front End Errors (completed) 
Step-16 - Final Code Optimization (completed)